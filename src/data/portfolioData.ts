export const experienceData = [
  {
    id: 1,
    company: "ADV Technology",
    position: "Team Leader & Ssr. Developer",
    period: "2022 - actualidad",
    periodEn: "2022 - present",
    description: "Liderazgo de equipo de desarrollo y arquitectura de soluciones web",
    descriptionEn: "Team leadership and web solutions architecture",
    technologies: ["C/C++", "HTML 5", "CSS 3", "JavaScript", "Oracle PL/SQL", "MS SQL Server"]
  },
  {
    id: 2,
    company: "DevAds",
    link: "https://devads.cloud",
    position: "Fundador, Desarrollo Web y Publicidad Online",
    positionEn: "Founder, Web Development and Online Advertising",
    period: "2017 - 2022",
    description: "Desarrollo de soluciones web personalizadas y estrategias de marketing digital",
    descriptionEn: "Custom web solutions development and digital marketing strategies",
    technologies: ["WordPress","Node.js","Express","TypeScript","Docker","Heroku","AWS", "PHP", "Google Ads" ,"Meta Ads","..."]
  }
];

export const projectsData = [
  {
    id: 1,
    title: "E-commerce Platform",
    titleEn: "E-commerce Platform",
    description: "Plataforma de comercio electrónico completa con panel de administración",
    descriptionEn: "Complete e-commerce platform with admin panel",
    image: "https://images.pexels.com/photos/230544/pexels-photo-230544.jpeg?auto=compress&cs=tinysrgb&w=800",
    technologies: ["React", "Node.js", "MongoDB", "Stripe"],
    year: "2023",
    github: "https://github.com",
    demo: "https://demo.com",
    gallery: [
      "https://images.pexels.com/photos/230544/pexels-photo-230544.jpeg?auto=compress&cs=tinysrgb&w=800",
      "https://images.pexels.com/photos/196644/pexels-photo-196644.jpeg?auto=compress&cs=tinysrgb&w=800"
    ]
  },
  {
    id: 2,
    title: "Sabore.ar Menu Digital",
    titleEn: "Sabore.ar Digital Menu",
    description: "Menu digital para restaurantes, sincronizado con google sheets",
    descriptionEn: "Digital menu for restaurants, synchronized with google sheets",
    image: "https://images.pexels.com/photos/5077068/pexels-photo-5077068.jpeg?auto=compress&cs=tinysrgb&w=800",
    technologies: ["React", "Mongo DB", "ExpressJS", "Heroku"],
    year: "2024",
    github: "https://github.com/Ecarcomo/Menu-Digital",
    demo: "https://sabore.ar/",
    gallery: [
      "https://images.pexels.com/photos/13432289/pexels-photo-13432289.jpeg?auto=compress&cs=tinysrgb&w=800",
      "https://images.pexels.com/photos/5077068/pexels-photo-5077068.jpeg?auto=compress&cs=tinysrgb&w=800"
    ]
  },
  {
    id: 3,
    title: "App Móvil Fitness",
    titleEn: "Fitness Mobile App",
    description: "Aplicación móvil para seguimiento de rutinas de ejercicio",
    descriptionEn: "Mobile app for workout routine tracking",
    image: "https://images.pexels.com/photos/6298479/pexels-photo-6298479.jpeg?auto=compress&cs=tinysrgb&w=800",
    technologies: ["React Native", "Firebase", "Redux"],
    year: "2022",
    github: "https://github.com",
    demo: "https://demo.com",
    gallery: [
      "https://images.pexels.com/photos/6298479/pexels-photo-6298479.jpeg?auto=compress&cs=tinysrgb&w=800",
      "https://images.pexels.com/photos/3768916/pexels-photo-3768916.jpeg?auto=compress&cs=tinysrgb&w=800"
    ]
  }
];

export const skillsData = {
    hard: [
    { category:"Tecnologías de Desarrollo", name: "React + Vite", icon: "⚛️" },
    { category:"Tecnologías de Desarrollo", name: "Express.js", icon: "⚡" },
    { category:"Tecnologías de Desarrollo", name: "Node.js", icon: "🟢" },
    { category:"Tecnologías de Desarrollo", name: "TypeScript", icon: "🔷" },
    { category:"Tecnologías de Desarrollo", name: "Python", icon: "🐍" },
    { category:"Tecnologías de Desarrollo", name: "PHP", icon: "🅿️" },
    { category:"Cloud", name: "AWS", icon: "☁️" },
    { category:"Cloud", name: "Firebase", icon: "🔥" },
    { category:"Cloud", name: "Meta Cloud", icon: "🔵" },
    { category:"Base De Datos", name: "MongoDB", icon: "🍃" },
    { category:"Base De Datos", name: "Supabase", icon: "🟢" },
    { category:"Base De Datos", name: "PostgreSQL", icon: "🐘" },
    { category:"Base De Datos", name: "Oracle PL/SQL", icon: "💻" },
    { category:"DevOps / Automatización", name: "N8n Workflows", icon: "" },
    { category:"DevOps / Automatización", name: "Docker", icon: "🐳" },
    { category:"DevOps / Automatización", name: "LLMs", icon: "🧠" },
    { category:"Deployment / Hosting", name: "Netlify", icon: "" },
    { category:"Deployment / Hosting", name: "Vercel", icon: "" },
    { category:"Deployment / Hosting", name: "GitHub Pages", icon: "" },
    { category:"Deployment / Hosting", name: "Hostinger", icon: "" },
    { category:"Deployment / Hosting", name: "Heroku", icon: "" },
  ],
  soft: [
    { name: "Comunicación", nameEn: "Communication", icon: "💬" },
    { name: "Creatividad", nameEn: "Creativity", icon: "🎨" },
    { name: "Trabajo en equipo", nameEn: "Teamwork", icon: "👥" },
    { name: "Liderazgo", nameEn: "Leadership", icon: "👑" },
    { name: "Resolución de problemas", nameEn: "Problem Solving", icon: "🧩" },
    { name: "Adaptabilidad", nameEn: "Adaptability", icon: "🔄" }
  ]
};

export const educationData = [
  {
    id: 1,
    institution: "UTN FRBA",
    degree: "Ingeniería Electrónica",
    degreeEn: "Electronic Engineering",
    period: "2016 - 2019",
    description: "Formación en sistemas electrónicos y programación",
    descriptionEn: "Training in electronic systems and programming"
  },
  {
    id: 2,
    institution: "Escuela N°32",
    degree: "Técnico Informático",
    degreeEn: "Computer Technician",
    period: "2010 - 2015",
    description: "Especialización en sistemas informáticos y redes",
    descriptionEn: "Specialization in computer systems and networks"
  },
  {
    id: 3,
    institution: "Instituto CENCAP",
    degree: "Inglés Pre-First",
    degreeEn: "English Pre-First",
    period: "2012 - 2018",
    description: "Certificación en idioma inglés nivel intermedio-avanzado",
    descriptionEn: "English language certification intermediate-advanced level"
  }
];

export const contactData = {
  email: "emmanuelcarcomo@gmail.com",
  phone: "+54 9 11 3765-8523",
  whatsapp: "5491137658523",
  linkedin: "https://www.linkedin.com/in/emmanuel-carcomo-2346a8113/",
  github: "https://github.com/Ecarcomo"
};